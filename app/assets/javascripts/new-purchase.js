...
if (cardForm) {
  ...
  // $( "#sky-form" ).submit(function( event ) {
    ...
    if (response.error) {
      ...
      $form.append($('<input type="hidden" name="address_one" />').val(response.card.address_line1));
      $form.append($('<input type="hidden" name="address_two" />').val(response.card.address_line2));
